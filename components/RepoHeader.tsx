...
import { Star, GitFork, Eye, Scale, Download, Copy, ChevronDown, Code, Bookmark, Github } from 'lucide-react';
    ...
    <header className="flex flex-col md:flex-row md:items-start justify-between gap-4">
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {isCodeOpen && (
            <div className="absolute left-0 md:left-auto md:right-0 top-full mt-2 w-80 bg-white dark:bg-base-900 border border-base-200 dark:border-base-700 rounded-xl shadow-xl z-20 overflow-hidden animate-fade-in">
              <div className="p-3">
                <div className="flex border-b border-base-200 dark:border-base-700 mb-2 -mx-1">
                    <TabButton name="https" activeTab={activeCloneTab} setActiveTab={setActiveCloneTab}>HTTPS</TabButton>
                    <TabButton name="ssh" activeTab={activeCloneTab} setActiveTab={setActiveCloneTab}>SSH</TabButton>
                    <button
                        onClick={() => setActiveCloneTab('cli')}
                ...
                <div className="relative">
                    {activeCloneTab === 'https' && (
                        <CloneInput value={cloneUrl} onCopy={() => handleCopy(cloneUrl, 'HTTPS URL copied!')} />
                    )}
                    {activeCloneTab === 'ssh' && (
                        <CloneInput value={sshUrl} onCopy={() => handleCopy(sshUrl, 'SSH URL copied!')} />
                    )}
                    {activeCloneTab === 'cli' && (
                        <CloneInput value={cliCommand} onCopy={() => handleCopy(cliCommand, 'CLI command copied!')} />
                    )}
                </div>
              </div>
              ...
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center w-full px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-base-100 dark:hover:bg-base-800 rounded-b-xl transition-colors"
              >
